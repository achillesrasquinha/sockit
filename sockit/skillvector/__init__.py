...
from sockit.data import (
    get_index,
    get_skill_idf_vector,
    get_soc,
    get_soc4,
    get_soc4_model,
    get_soc_skill_matrix,
    get_soc_title,
    get_soc4_title,
)
...
class SkillVector:
    ...
    def __len__(self):
        return len(
            [
                skill
                for skill in self.skill_counter.keys()
                if self.skill_counter[skill] >= 1
            ]
        )
    ...
    def __iter__(self):
        for skill, count in sorted(
            self.skill_counter.items(), key=lambda x: x[1], reverse=True
        ):
            yield (skill, count)
    ...
    def to_array(self):
        return np.array([self.get(x, 0) for x in sorted(list(self.index.keys()))])
